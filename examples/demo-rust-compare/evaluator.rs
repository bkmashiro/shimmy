...
// Deliberately mutable guest state. Shimmy snapshots this after startup and
// restores it after every request, so each call should observe count == 1.
...
pub extern "C" fn dispatch(_req_ptr: i32, req_len: i32) -> i32 {
    ...
    let correct_feedback =
        json_string_field(req, b"correct_response_feedback").unwrap_or(b"Correct!");
    let incorrect_feedback =
        json_string_field(req, b"incorrect_response_feedback").unwrap_or(b"Try again.");

    let is_correct = response.is_some() && answer.is_some() && response == answer;
...
fn json_string_field<'a>(input: &'a [u8], name: &[u8]) -> Option<&'a [u8]> {
    ...
    let mut i = key + name.len() + 2; // leading quote + name + trailing quote
    while i < input.len()
