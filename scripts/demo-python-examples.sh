...
ensure_reactor_wasm() {
  if [[ ! -f "${SHIMMY_REACTOR_WASM}" ]]; then
    echo "error: pinned Shimmy Python artifact is missing: ${SHIMMY_REACTOR_WASM}" >&2
    exit 1
  fi
  printf '%s\n' "${SHIMMY_REACTOR_WASM}"
}
...
run_plain_reactor() {
  echo
  echo "==> Plain Python route: examples/eval-python via shimmy-python"
...
run_numpy_reactor() {
  ...
  echo
  echo "==> NumPy route: examples/eval-numpy via shimmy-python"
...
main() {
  ...
  if [[ "${mode}" == "reactor-only" ]]; then
    ...
    run_numpy_reactor
    echo
    echo "✅ Shimmy Python example demos completed. Logs: ${LOG_DIR}"
    return 0
  fi
