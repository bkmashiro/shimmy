...
class LaneAssertionTests(unittest.TestCase):
    ...
    def test_shimmy_python_cow_requires_correct_result(self):
        module.assert_lane_response(
            "shimmy-python-cow",
        ...
        spec = module.LANES["shimmy-python-cow"]
        self.assertIn("COW", spec.lifecycle)
        self.assertIn("runtime_prepare", spec.initialization_placement)

    def test_shimmy_python_cow_requires_selected_cow_healthcheck(self):
        evidence = module.assert_shimmy_python_cow_healthcheck(
            {
                "command": "healthcheck",
                "result": {
                    "lifecycle": "snapshot",
        ...
        with self.assertRaisesRegex(ValueError, "snapshot_selected"):
            module.assert_shimmy_python_cow_healthcheck(
                {
                    "result": {
                        "lifecycle": "snapshot",
...
class ComposeLifecycleContractTests(unittest.TestCase):
    def test_shimmy_python_benchmark_image_pins_snapshot_cow(self):
