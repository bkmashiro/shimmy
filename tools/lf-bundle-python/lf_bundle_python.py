...
def runtime_modules_from_manifest(path: Path | None) -> list[str]:
    if path is None:
        return []
    document = json.loads(path.read_text())
    if document.get("schema") != "shimmy-python-runtime-artifact/v1":
        raise ValueError("runtime manifest has unsupported schema")
    if document.get("artifact_contract") != "shimmy-python-runtime/v1":
        raise ValueError("runtime manifest has unsupported artifact contract")
    expected = {
        "base": [],
        "numpy-core": ["numpy"],
        "sympy": ["mpmath", "sympy"],
    ...
    modules = document.get("python_modules")
    if profile not in expected or modules != expected[profile]:
        raise ValueError("runtime manifest python_modules do not match its profile")
    return list(modules)
...
def validate_dependencies(
    ...
    raise ValueError(
        "unresolved imports: "
        + details
        + "; add pure-Python source with --include-root or select an artifact manifest that declares the module"
    )
...
def preview_function(response, params):
    return dispatch("preview", {"response": response, "params": params})
""" if preview_entrypoint else ""
...
def evaluation_function(response, answer, params):
    return dispatch("eval", {{"response": response, "answer": answer, "params": params}})
{preview_wrapper}'''
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--runtime-manifest",
        type=Path,
        help="Bound Shimmy Python artifact manifest used to satisfy artifact-provided imports",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        bundle = build_bundle(
            ...
            runtime_modules=runtime_modules_from_manifest(args.runtime_manifest),
        )
